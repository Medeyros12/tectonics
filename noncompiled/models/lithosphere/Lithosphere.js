...
function Lithosphere(grid, parameters) {
    ...
    this.calcChanges = function(seconds) {
        var max_perceivable_duration = 60*60*24*30 * seconds; 
        if (mean_supercontinent_cycle_duration > max_perceivable_duration) { return; }
        assert_dependencies();
        calculate_deltas(this, seconds);
    };
    ...
    this.applyChanges = function(seconds){
        ...
        integrate_deltas         (this, this.plates, seconds);
        move_plates             (this.plates, seconds); 
        if (this.enablePlateCycling) {
            this.supercontinentCycle.update(seconds);
        }
        merge_plates_to_master    (this.plates, this);
        update_rifting            (this, this.plates);
        update_subducted         (this, this.plates);
    };
}
